...
const addProductFunctionDeclaration: FunctionDeclaration = {
  name: 'addProduct',
  description: 'إضافة صنف جديد إلى القائمة',
  parameters: {
    type: Type.OBJECT,
    properties: {
...
const updateProductFunctionDeclaration: FunctionDeclaration = {
  name: 'updateProduct',
  description: 'تعديل بيانات صنف موجود. يجب تحديد الصنف بالاسم أو بالمعرف.',
  parameters: {
    type: Type.OBJECT,
    properties: {
      id: { type: Type.STRING, description: 'معرف الصنف المطلوب تعديله' },
...
const deleteProductFunctionDeclaration: FunctionDeclaration = {
  name: 'deleteProduct',
  description: 'حذف صنف من القائمة بشكل نهائي.',
  parameters: {
    type: Type.OBJECT,
    properties: {
      id: { type: Type.STRING, description: 'معرف الصنف المطلوب حذفه' },
    },
    required: ['id'],
  },
};
    ...
    try {
      ...
      if (!chatRef.current) {
        const systemInstruction = `
          أنت مساعد أعمال ذكي ومتكامل باللغة العربية. مهمتك هي مساعدة المستخدم في إدارة أعمال مطعمه "مطعم الباشا" بشكل كامل.
          لديك القدرة على:
          ...
          - للبحث عن عنصر (صنف, مصروف, عميل) لتعديله أو حذفه، استخدم البيانات المتاحة في السياق. إذا لم تجده، أخبر المستخدم بذلك.
        `;
        ...
        for (const funcCall of response.functionCalls) {
            ...
            try {
                if (funcCall.name === 'addProduct') {
                    ...
                    addProduct(args);
                    result = { success: true, message: `تمت إضافة الصنف ${args.name} بنجاح.` };
                    ...
                    if (existingProduct) {
                        ...
                        result = { success: true, message: `تم تحديث بيانات الصنف ${updatedData.name} بنجاح.` };
                    } else {
                        result = { success: false, message: `لم يتم العثور على صنف بالمعرف ${args.id}.` };
                    }
                } else if (funcCall.name === 'deleteProduct') {
                    const { id } = funcCall.args as { id: string };
                    deleteProduct(id);
                    result = { success: true, message: `تم حذف الصنف بالمعرف ${id} بنجاح.` };
                    ...
                    const { items } = funcCall.args as { items: { productId: string }[] };
            ...
            {messages.length === 0 && !isLoading && (
                 <div className="text-center p-4 text-slate-500 animate-[fade-in_0.5s_ease-out]">
                    ...
                    <div className="text-xs mt-4 space-y-2 text-left bg-white p-3 rounded-lg border">
                        ...
                        <button onClick={() => handleSendMessage('ما هي مبيعات اليوم؟')} className="block w-full text-right text-indigo-600 hover:underline">"ما هي مبيعات اليوم؟"</button>
                        <button onClick={() => handleSendMessage("عدّل سعر 'شاورما دجاج' إلى 90")} className="block w-full text-right text-indigo-600 hover:underline">"عدّل سعر 'شاورما دجاج' إلى 90"</button>
                        ...
                        <button onClick={() => handleSendMessage("أنشئ فاتورة بيع لـ 'مشويات مشكلة'")} className="block w-full text-right text-indigo-600 hover:underline">"أنشئ فاتورة بيع لـ 'مشويات مشكلة'"</button>
                    </div>
                </div>
            )}
