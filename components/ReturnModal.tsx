...
const ReturnModal: React.FC<ReturnModalProps> = ({ invoice, onClose, onProcessReturn }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (returnItems.length === 0) {
      alert('الرجاء تحديد صنف واحد على الأقل للإرجاع.');
      return;
    }
      ...
      <div className="bg-white rounded-lg shadow-xl w-full max-w-lg">
        ...
        <form onSubmit={handleSubmit}>
          <div className="p-6 max-h-[60vh] overflow-y-auto">
            <p className="text-sm text-gray-600 mb-4">حدد الأصناف التي تريد إرجاعها.</p>
            <div className="space-y-2">
                {invoice.items.map((item, index) => (
                    <div 
                        key={`${item.productId}-${index}`} 
                        className={`flex items-center justify-between p-3 border rounded-xl cursor-pointer transition-colors ${selectedIndices.includes(index) ? 'bg-red-50 border-red-200' : 'bg-white border-slate-100 hover:bg-slate-50'}`}
                        onClick={() => toggleItem(index)}
                    >
                        <div className="flex items-center gap-3">
                            <div className={`w-5 h-5 rounded border flex items-center justify-center transition-colors ${selectedIndices.includes(index) ? 'bg-red-600 border-red-600 text-white' : 'border-slate-300 bg-white'}`}>
